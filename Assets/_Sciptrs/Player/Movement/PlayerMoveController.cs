...
namespace MyGame
{
    ...
    [System.Serializable]
    public class PlayerMoveController : MovementController, IMoveObservable, IPushbackTarget
    {
        [SerializeField] private bool InspectorSetings = false;
        [SerializeField] private PawnMoveSettings _settings;
        [Space(5)]
        [SerializeField] private InputClickChannelSO _inputChannel;
        [Space(5)]
        ...
        public void OnClick()
        {
            Ray ray = Camera.main.ScreenPointToRay(_inputChannel.MousePosition);
            if (Physics.Raycast(ray, out RaycastHit hit, 100, _floorLayers))
            {
                Vector3 targetPos = hit.point;
                Vector3 allowedPos = _positionValidator.GetCorrectedPosition(targetPos,_mover.CurrentPosition, _settings.DeadZone);
                StartMoving(allowedPos, targetPos);
            }
        }
